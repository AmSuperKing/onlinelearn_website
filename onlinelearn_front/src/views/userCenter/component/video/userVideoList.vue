  ...
  data () {
    return {
      ...
      rules: {
        ...
        desc: [
          { required: true, message: '请填写教程描述', trigger: 'blur' }
        ]
      }
    }
  },
  ...
  methods: {
    ...
    initListData () {
      ...
      if (userId) {
        ...
        videoInfoApi
          .getMyVideoList(this.pageParams)
          .then(res => {
            // console.log('视频数据列表', res.data)
    ...
    commitChange () {
      if (this.changeForm.name && this.changeForm.desc) {
        if (this.changeForm.path) {
          videoInfoApi
            .updateVideoInfo(this.changeForm)
            .then(res => {
              // console.log('updateVideoInfo', res.data)
    ...
    searchVideo () {
      if (this.pageParams.name ||
        ...
        videoInfoApi
          .searchVideoByKey(this.pageParams)
          .then(res => {
