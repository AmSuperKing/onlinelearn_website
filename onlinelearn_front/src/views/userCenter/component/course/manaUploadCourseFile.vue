    ...
    <div class="form-box">
      <el-form :model="uploadForm" :rules="rules" ref="uploadForm" label-width="80px" class="demo-ruleForm">
        <el-form-item label="选择课程" prop="course">
          <el-select v-model="uploadForm.course" placeholder="请选择上传的课程">
            <el-option
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        course: [
          { required: true, message: '请选择上传的课程', trigger: 'change' }
        ],
        fileName: [
  ...
  methods: {
    ...
    submitUpLoadForm (formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          courseInfoApi
            .uploadCourseFile(this.uploadForm)
            .then(res => {
