  ...
  <div>
    ...
    <el-row>
      <div v-show="noHaveContent" class="no-content">
        ...
        <el-button size="small" type="primary" class="show-all-btn" @click="getJoinCourseList">返回全部列表</el-button>
      </div>
    </el-row>
  </div>
</template>
  ...
  methods: {
    getJoinCourseList () {
      ...
      if (userId) {
        ...
        courseInfoApi
          .getJoinCourseList(this.pageParams)
          .then(res => {
            // console.log('我加入的列表', res.data)
    ...
    searchCourse () {
      ...
      if (this.searchWord.length) {
        courseInfoApi
          .searchJoinCourse(userId, this.searchWord)
          .then(res => {
